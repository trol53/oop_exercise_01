...
class Queue {
private:
    struct Item; //struct for node
    class QueueIterator; //class iterator
    std::shared_ptr<Item> F_elem;
    int size;
    TAllocator<T,100> all;
public:
    using value_type = T; 
    Queue() : size(0){ //basic constructor
    ...
    QueueIterator Begin() { //function for return first iterator
        if (this->Empty()){
            return QueueIterator(F_elem, this);
        }
        return QueueIterator(F_elem->next, this);
    }
    QueueIterator End() { //function for return last iterator
        return QueueIterator (F_elem, this);
    }
...
private:
    ...
    public:
        ...
        QueueIterator(const QueueIterator& other){ //copy constructor
            item = other.item;
            Ths = other.Ths;
        }
    ...
    void InsertP(std::shared_ptr<Item> node, T val){ //function for insert element on iterator
        if (size == 0){
            EmptyInsert(val);
            size++;
            return;
    ...
    void Remove (std::shared_ptr<Item> node){ //function for erase element on iterator
        if (size == 1){
            F_elem->next = nullptr;
        } else {
            if (size == 2){
